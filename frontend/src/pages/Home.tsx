...
const CATEGORY_LABEL: Record<string, { label: string; color: string }> = {
  ...
  industry:    { label: '行业',   color: 'bg-orange-500/20 text-orange-400' },
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {mineralsWithPrice.slice(0, 8).map(m => {
              ...
              return (
                <div key={m.id} className="bg-stone-800/60 rounded-lg p-3">
                  ...
                  <div className="flex items-center justify-between mt-1">
                    <span className="text-xs text-stone-500">{m.latest_price?.unit}</span>
        ...
        <div className="card">
          ...
          <div className="flex flex-wrap gap-3">
            {Object.entries(stats.news_by_category).map(([cat, count]) => {
              const cfg = CATEGORY_LABEL[cat] ?? { label: cat, color: 'bg-stone-700 text-stone-300' }
              return (
        ...
        <div className="card">
          ...
          <ul className="space-y-3">
            {stats!.recent_news.map(n => {
              const cfg = CATEGORY_LABEL[n.category] ?? { label: n.category, color: 'bg-stone-700 text-stone-300' }
              return (
                <li key={n.id} className="flex items-start gap-3 py-2 border-b border-stone-800 last:border-0">
                  <span className={`badge mt-0.5 shrink-0 ${cfg.color}`}>{cfg.label}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-stone-200 leading-snug line-clamp-2">{n.title}</p>
