...
const EXCHANGE_COLOR: Record<string, string> = {
  ...
  OTC:   'bg-slate-500/20 text-slate-400',
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <span className="text-xs text-slate-500">矿产筛选:</span>
            ...
            return (
              <div key={c.id} className="card flex flex-col gap-3">
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-xs px-1.5 py-0.5 rounded ${EXCHANGE_COLOR[c.exchange ?? ''] ?? 'bg-slate-700 text-slate-400'}`}>
                        {c.ticker}
                      </span>
                    </div>
                    <p className="text-xs text-slate-500 mt-0.5">{c.country} · {c.exchange}</p>
                  </div>
                  {c.website && (
                ...
                {snap ? (
                  ...
                  <div className="bg-slate-800/40 rounded-lg px-3 py-2 text-xs text-slate-600">
                    暂无行情数据
                  </div>
                )}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(c.minerals_focus ?? []).map(m => (
                    <span key={m} className="text-xs bg-slate-800 text-slate-400 px-2 py-0.5 rounded">
                      {m}
                    </span>
                  ))}
                </div>
