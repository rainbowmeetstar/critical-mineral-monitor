...
const CATEGORY_STYLE: Record<string, string> = {
  ...
  industry:    'bg-sky-500/15 text-sky-400 border-sky-500/30',
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="card flex flex-wrap gap-4 items-end">
        <div>
          <label className="block text-xs text-slate-500 mb-1.5">类别</label>
            ...
            {CATEGORY_OPTIONS.map(o => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs transition-colors ${
                  category === o.value ? 'bg-sky-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'
                }`}
              >
                {o.label}
              </button>
        ...
        <div>
          <label className="block text-xs text-slate-500 mb-1.5">层级</label>
          <div className="flex gap-1 flex-wrap">
            {LEVEL_OPTIONS.map(o => (
              <button
                key={o.value}
                ...
                className={`px-3 py-1.5 rounded-lg text-xs transition-colors ${
                  level === o.value ? 'bg-sky-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'
                }`}
              >
                {o.label}
              </button>
        ...
        <div>
          <label className="block text-xs text-slate-500 mb-1.5">矿产关键词</label>
          <input
            value={mineral}
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {articles!.map(a => {
            const catStyle = CATEGORY_STYLE[a.category ?? ''] ?? 'bg-slate-700/50 text-slate-400 border-slate-600'
            return (
                ...
                <div className="flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-2">
                      ...
                      {a.country && (
                        <span className="text-xs text-slate-600">{a.country}</span>
                      )}
                    </div>
                    ...
                    <a
                      ...
                      className="group"
                    >
                      <h3 className="text-sm font-medium text-slate-200 group-hover:text-sky-300 transition-colors leading-snug">
                        {a.title}
                      ...
                      {(a.minerals_mentioned?.length ?? 0) > 0 && (
                        <div className="flex gap-1 ml-auto">
                          {a.minerals_mentioned!.slice(0, 4).map(m => (
                            <span key={m} className="badge bg-slate-800 text-slate-400 text-[10px]">
                              {m}
                            </span>
                          ))}
                        </div>
      ...
      <div className="flex gap-2 justify-center pt-2">
        ...
        <span className="flex items-center text-sm text-slate-400">第 {page} 页</span>
        <button
